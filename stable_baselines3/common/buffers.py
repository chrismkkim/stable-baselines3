...
from stable_baselines3.common.type_aliases import (
    ...
    RolloutDopaBufferSamples,
    MetaRolloutBufferSamples,
)
...
class RolloutDopaBuffer(BaseBuffer):
    """
    Rollout buffer used in on-policy algorithms like A2C/PPO.
    It corresponds to ``buffer_size`` transitions collected
    using the current policy.
    ...
    def reset(self) -> None:
        ...
        self.dones = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.next_dones = np.zeros((self.buffer_size, self.n_envs), dtype=np.bool)
        self.log_probs = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.advantages = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.dopa = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.returns_dopa = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.generator_ready = False
        super().reset()
    ...
    def compute_returns_and_advantage(self, last_values: th.Tensor, dones: np.ndarray) -> None:
        ...
        for step in reversed(range(self.buffer_size)):
            ...
            self.next_dones[step] = (1.0 - next_non_terminal).astype(bool)
        # TD(lambda) estimator, see Github PR #375 or "Telescoping in TD(lambda)"
        # in David Silver Lecture 4: https://www.youtube.com/watch?v=PnHCvfgC_ZA
        self.returns = self.advantages + self.values
        self.returns_dopa = self.dopa + self.values
    ...
    def add(
        ...
        :param log_prob: log probability of the action
            following the current policy.
        :param dopa: output from dopa network
        """
        if len(log_prob.shape) == 0:
            # Reshape 0-d tensor to avoid error
            log_prob = log_prob.reshape(-1, 1)
        ...
        self.values[self.pos] = value.clone().cpu().numpy().flatten()
        self.log_probs[self.pos] = log_prob.clone().cpu().numpy()
        self.dopa[self.pos-1] = dopa.clone().cpu().numpy().flatten()
        self.pos += 1
        if self.pos == self.buffer_size:
            self.full = True
    ...
    def add_last_dopa(
        ...
        """
        :param dopa: output from dopa network at the last timestep
        """
        self.dopa[self.pos-1] = dopa.clone().cpu().numpy().flatten()
    ...
    def _get_samples(
        ...
        data = (
            ...
            self.actions[batch_inds],
            self.values[batch_inds].flatten(),
            self.next_values[batch_inds].flatten(),
            self.next_dones[batch_inds].flatten(),
            self.log_probs[batch_inds].flatten(),
            self.dopa[batch_inds].flatten(),
            self.advantages[batch_inds].flatten(),
            self.returns[batch_inds].flatten(),
...
class MetaRolloutBuffer(BaseBuffer):
    """
    Rollout buffer used in on-policy algorithms like A2C/PPO.
    It corresponds to ``buffer_size`` transitions collected
    using the current policy.
    ...
    def get(self, batch_size: Optional[int] = None) -> Generator[MetaRolloutBufferSamples, None, None]:
        assert self.full, ""
        indices = np.random.permutation(self.buffer_size * self.n_envs)
        # Prepare the data
        if not self.generator_ready:
    ...
    def _get_samples(
        ...
        return MetaRolloutBufferSamples(*tuple(map(self.to_torch, data)))
